...
calvis.MONTHS = [ 'January', 'February', 'March', 'April', 'May', 'June', 'July', 'August',
		'September', 'October', 'November', 'December' ];

// Constant strings for day labels
calvis.ABBRDAYS = [ 'Sun', 'Mon', 'Tues', 'Wed', 'Thurs', 'Fri', 'Sat' ];
calvis.DAYS = [ 'Sunday', 'Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday' ];
 ...
 * 
 * @param {string}
 *            trigger The event that triggers the callback, can be "click" or
 *            "mouseover"
 * @param {Function}
 *            callback This callback will be invoked when the trigger occurs.
 ...
 *            This callback should use this event object and access its details
 *            for display
 */
calvis.Calendar.prototype.setEventCallback = function(trigger, callback) {
 ...
 * 
 * @param {string}
 *            view The View setting, either "month" or "week".
 */
...
calvis.Calendar.prototype.initWeekNavControl = function() {
	...
	jQuery('#' + calvis.prevWeekButtonId).click(
			function() {
				calendar.currentPivotDate.setDate(calendar.currentPivotDate
						.getDate() - 7);
				calendar.updateWeekView();
			});

	jQuery('#' + calvis.nextWeekButtonId).click(
			function() {
				calendar.currentPivotDate.setDate(calendar.currentPivotDate
						.getDate() + 7);
				calendar.updateWeekView();
...
calvis.Calendar.prototype.updateWeekView = function() {
	...
	for ( var i = 0; i < 7; i++) {
		...
		weekDateHeading.push(calvis.abbrDayString(dateHolder.getDay()));
	...
	for ( var i = 0; i < 7; i++) {
		...
		if (dateCellId == this.currentDateCellID)
			dateHtml.push(calvis.weekViewCellClass + ' today');
		else
...
calvis.Calendar.prototype.createYearChooser = function() {
	...
	var element = jQuery(html).change(
			function() {
				calendar.currentPivotDate.setFullYear($(this).get(0).options[$(
						this).get(0).selectedIndex].value);
...
calvis.Calendar.prototype.updateMonthNavControl = function() {
	...
	jQuery('#' + calvis.monthHolderId).html(
			calvis.monthString(this.currentPivotDate.getMonth()));
};
...
calvis.Calendar.prototype.updateMonthView = function() {
	...
	for ( var i = 0; i < daysInMonth; i++) {
		...
		// add a class to todays date cell
		if (dateCellId == this.currentDateCellID)
			dateHtml.push(calvis.monthViewCellClass + ' today');
		else
		...
		var dateString = (dateHolder.getMonth() + 1) + "/" + dateHolder.getDate();
	

		dateHtml.push('<span class="date">' + dateString + '</span>');
...
/**
 * This is an internal callback that is invoked when the JavaScript client
 * library returns the event data. It will then display the event title on its
 * corresponding date cell.
 * 
 * @param {string}
 *            id The date cell.
 ...
 * @param {string}
 *            label A div ID identifying so that each calendar may have a
 *            different style attached to its events
 * 
 */
calvis.Calendar.prototype.appendEvent = function(id, event, label) {
	...
	if (cellHeight < currentContentHeight + contentFontHeight + bottomMargin) {
		if (cell.get(0).lastChild != null
				&& cell.get(0).lastChild.innerHTML == calvis.moreLabel) {
		...
		moreDiv.bind('click', function() {
			calendar.currentPivotDate = event.getTimes()[0].getStartTime()
					.getDate();
		...
		for (i = 0; i < times.length; i++) {
			...
			var days = Math.floor(times[i].getEndTime().getDate()
					- times[i].getStartTime().getDate())
					/ (1000 * 60 * 60 * 24);
			...
			for (j = 0; j < days; j++) {
				...
				if (days > 1) {
					if (j == 0)
						eventHtml.push(calvis.eventClass + ' first');
					else if (j == days - 1)
						eventHtml.push(calvis.eventClass + ' last');
					else
						eventHtml.push(calvis.eventClass + ' middle');
				} else {
				...
				for (var j = 0; j < eventTimes.length; ++j) {
				  ...
				  var endTime = eventTimes[j].getEndTime();
					...
					if (!startTime.isDateOnly()) {
						hoursString += " " + formatAMPM(date);
					}
					if (!endTime.isDateOnly()) {
						hoursString += " - " + formatAMPM(enddate);
					}
					
					//alert(date);
				  //alert(startTime.getDate());
				  // Do what you want with the start and end times.
				}
			
				
				eventHtml.push(calendar.fitText(hoursString,
						cellWidth));
				eventHtml.push('</div>');
...
/**
 * When the date cell is overflowed because of too many events for a particular
 * date, a "more" link is created.
 * 
 * @param {string}
 *            id The date cell ID which this more link belongs to.
 */
...
/**
 * This method overlays the event data between two dates and append these events
 * on their appropriate date cells.
 * 
 * @param {Date}
 ...
 * @param {string}
 *            label An identifer used to style events per calendar
 */
...
calvis.abbrDayString = function(offset) {
	return calvis.ABBRDAYS[offset];
};
 ...
 * 
 * @param {Date}
 *            pivotDate The current pivot date to locate the current week.
 */
calvis.getFirstDateOfWeek = function(pivotDate) {
	var dayOffset = pivotDate.getDay();
	return new Date(pivotDate.getFullYear(), pivotDate.getMonth(), pivotDate
			.getDate()
			- dayOffset);
};
 ...
 * 
 * @param {string}
 *            text The text that is examined to fit it width to date cell.
 * @param {number}
 *            limit The maximum number of characters that is permitted.
 */
...
calvis.handleError = function(e) {
	...
	if (e instanceof Error) {
		...
		jQuery('#' + this.statusControlId).html(
				'Error at line ' + e.lineNumber + ' in ' + e.fileName + '\n'
						+ 'Message: ' + e.message + '.<br />');
		/* if available, output HTTP error code and status text */
		if (e.cause) {
			var status = e.cause.status;
			var statusText = e.cause.statusText;
			jQuery('#' + this.statusControlId).append(
					'Root cause: HTTP error ' + status
...
function OnLoad() {
	...
	var calIds = [ 'j77889blek0m438pp3ae22tbu8@group.calendar.google.com' ];
