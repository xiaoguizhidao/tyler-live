    ...
    if(productId) {
        new Ajax.Request(imgUrl, {
            ...
            onComplete: function(response) {
				...
				//Don't display if there is no image, switch back to configurable images.
				if(!response.responseText.match(/placeholder/)) {
					$(destElement).update(response.responseText);
				}
				else
				{
                ...
                if ($('image')){
					...
					jQuery(function($){
						...
						$('.cloud-zoom, .cloud-zoom-gallery').die('hover');
					});				
