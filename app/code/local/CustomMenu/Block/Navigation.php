...
class Peerforest_CustomMenu_Block_Navigation extends Mage_Catalog_Block_Navigation
{
    const CUSTOM_BLOCK_TEMPLATE = "wp_custom_menu_%d";

    private $_productsCount = null;
    private $_activeCategory = false;
    ...
    public function getHomeActiveClass($categories){
        $actClass = "";
    ...
    public function drawCustomMenuItem($category, $level = 0, $last = false)
    {
        ...
        $html[] = '<span>' . $name . '</span><div class="drop-active visible-onhover"></div>';
        if($category->getData('category_label')){
            $html[] = '<span class="category-label">' . $category->getData('category_label') . '</span>';
        }
        $html[] = '</a>';
        ...
        $html[] = '</div>';
        // --- Add Popup block (hidden) ---
        $blockEnable    = Mage::getStoreConfig('mygeneral/menu/enable');
        $blockPosition  = $category->getData('category_block_position');
        $block1Class = "";
        if($blockHtml && ($blockPosition == 'left' || $blockPosition == 'right'))
        {
            //$block1Class = "grid12-8";
            //$block2Class = "grid12-4";
        }
        if ($drawPopup) {
            ...
            // --- draw Sub Categories ---
            if (($blockHtml && $blockEnable) && ($blockPosition == 'top')) {                
                $html[] = '<div id="' . $blockId . '" class="block2 '.$blockPosition.'">';
                $html[] = $blockHtml;
                $html[] = '</div>';
            }
            if (count($activeChildren)) {
                $columns = (int)Mage::getStoreConfig('custom_menu/columns/count');
                $html[] = '<div class="block1 '.$block1Class.'">';
                $html[] = $this->drawColumns($activeChildren, $columns);
                $html[] = '<div class="clearBoth"></div>';
                $html[] = '</div>';
            }
            // --- draw Custom User Block ---
            if (($blockHtml && $blockEnable) && ($blockPosition == 'bottom')) {
                $html[] = '<div id="' . $blockId . '" class="block2 '.$blockPosition.'">';
                $html[] = $blockHtml;
                $html[] = '</div>';
            }
    ...
    public function drawMobileMenuItem($children, $level = 1)
    {
        ...
        foreach ($children as $child) {
            if (is_object($child) && $child->getIsActive()) {
                ...
                $html.= '<div class="parentMenu">';
                $html.= '<a class="itemMenuName level' . $level . '" href="' . $this->getCategoryUrl($child) . '"><span>' . $name . '</span>';
    ...
    public function drawMenuItem($children, $level = 1)
    {
        ...
        foreach ($children as $child)
        {
            if (is_object($child) && $child->getIsActive())
            {
                if($ct == 0 && $level == 1 && Mage::getStoreConfig('mygeneral/menu/categoryimage') == 1 && $this->_getThumbnailImageUrl($child->getId()) != false)
                {   
                    $html .= '<div class="category-thumb"><img src="'.$this->_getThumbnailImageUrl($child->getId()).'" /></div>';
                    $ct=1;
                }
                ...
                if (Mage::getStoreConfig('custom_menu/general/non_breaking_space'))
                    $name = str_replace(' ', '&nbsp;', $name);
                $html.= '<a class="itemMenuName level' . $level . $active . '" href="' . $this->getCategoryUrl($child) . '"><span>' . $name . '</span>';
                if($child->getData('category_label')){
                $html.= '<span class="category-label">' . $child->getData('category_label') . '</span>';
                }
                $html.= '</a>';
    ...
    protected function _getThumbnailImageUrl($categoryKey)
    {
        $url = false;
        //$url = Mage::getBaseUrl('media').'wysiwyg/category-banner/category-default.jpg';
        
        if ($image = Mage::getModel('catalog/category')->load($categoryKey)->getThumbnail()) {
