 ...
 */
     ...
     *
     * Modified to return configurable parent URL.
     *
     * @param  bool $useSid
     * @return string
     */
    public function getProductUrl($useSid = null)
    {
        //Tags
        $tagValue = 'visible';
        $tags = strtolower($this->getResource()->getAttribute('tags')->getFrontend()->getValue($this));
        $tags = explode(',', $tags);
        $tags = array_map('trim', $tags);

        if(array_search($tagValue, $tags) !== FALSE && $this->getTypeId() == Mage_Catalog_Model_Product_Type::TYPE_SIMPLE)
        {
            $rewrite = Mage::getModel('core/url_rewrite');
            $params = array();
            $params['_current'] = false;
            $params['_query'] = array('sel' => $this->getId());
			...
			//Check and make sure modified url exists.
			if(isset($requestPath) && $requestPath !== '')
            	return Mage::getUrl($rewrite->getRequestPath(), $params);
			else
			{
				$parentProduct = Mage::getModel('catalog/product')->load($parentId);
				$params['useSid'] = $useSid;
				$parentUrl = $this->getUrlModel()->getUrl($parentProduct, $params);
				
				if(isset($parentUrl) && $parentUrl !== '')
					return $parentUrl;
			}
        }
